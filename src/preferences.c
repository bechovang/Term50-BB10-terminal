...
static char** create_string_array(config_t const *config, char const *path, size_t def_len, char const *const *def) {
	config_setting_t *setting = config_lookup(config, path);
	int use_default = 0;
	size_t source_len = 0;
	...
	char **result = calloc(source_len + 1, sizeof(char*));
	for (size_t i = 0; i < source_len; ++i) {
		char const *value = use_default ? def[i] : config_setting_get_string_elem(setting, (unsigned int)i);
		if (value != NULL) {
...
static keymap_t* create_keymap_array(config_t const *config, char const *path, size_t def_len, keymap_t const *def) {
	...
	result[source_len] = (keymap_t){0, NULL, NULL}; // sentinel for end of array
	
	if (use_default) {
		for (int i = 0; i < source_len; i++) {
			result[i].from = def[i].from;
			result[i].label = def[i].label ? strdup(def[i].label) : NULL;
		...
		for (int i = 0; i < source_len; i++) {
			...
			int elem_len = config_setting_length(m);
			result[i].from = from_str[0];
			if (elem_len >= 3) {
				result[i].label = strdup(config_setting_get_string_elem(m, 1));
				result[i].to = strdup(config_setting_get_string_elem(m, 2));
			} else {
...
static symmenu_t* create_symmenu(config_t const *config, char const *path, int def_num_rows, int const *def_row_lens, keymap_t const *def_entries) {
	...
	if (!rows_s || (config_setting_type(rows_s) != CONFIG_TYPE_LIST)) {
		...
		menu->entries[def_num_keys] = (keymap_t){0, NULL, NULL}; // sentinel for end of array
		...
		for (int row = 0; row < def_num_rows; ++row) {
			...
			for (int col = 0; col < def_row_lens[row]; ++col) {
				...
				menu->entries[entry_idx].label = def_entries[entry_idx].label ? strdup(def_entries[entry_idx].label) : NULL;
		...
		menu->entries[num_keys] = (keymap_t){0, NULL, NULL}; // sentinel for end of array
		...
		for (int row = 0; row < config_setting_length(rows_s); ++row) {
			...
			for (int col = 0; col < col_len; ++col) {
				...
				if (elem_len >= 3) {
					menu->entries[entry_idx].label = strdup(config_setting_get_string_elem(m, 1));
					menu->entries[entry_idx].to = strdup(config_setting_get_string_elem(m, 2));
				} else {
...
static symmenu_t* create_symmenu_from_keymap(keymap_t const *source, int row_width) {
	int num_keys = 0;
	while (source[num_keys].to != NULL) {
		++num_keys;
	}
	...
	if (num_keys == 0) {
		menu->keys = calloc(1, sizeof(symkey_t*));
		return menu;
	}

	int num_rows = (num_keys + row_width - 1) / row_width;
	menu->entries = calloc(num_keys + 1, sizeof(keymap_t));
	menu->entries[num_keys] = (keymap_t){0, NULL, NULL};
	menu->keys = calloc(num_rows + 1, sizeof(symkey_t*));
	menu->keys[num_rows] = NULL;

	int entry_idx = 0;
	for (int row = 0; row < num_rows; ++row) {
		int row_len = row_width;
		if ((entry_idx + row_len) > num_keys) {
			row_len = num_keys - entry_idx;
		}

		menu->keys[row] = calloc(row_len + 1, sizeof(symkey_t));
		menu->keys[row][row_len].map = NULL;
		for (int col = 0; col < row_len; ++col) {
			menu->entries[entry_idx].from = source[entry_idx].from;
			menu->entries[entry_idx].label = source[entry_idx].label ? strdup(source[entry_idx].label) : NULL;
			menu->entries[entry_idx].to = strdup(source[entry_idx].to);
			menu->keys[row][col].map = &menu->entries[entry_idx];
			++entry_idx;
		}
	}
...
void destroy_preferences(pref_t *pref) {
	...
	if (pref->fallback_font_paths != NULL) {
		for (int i = 0; pref->fallback_font_paths[i] != NULL; ++i) {
			free(pref->fallback_font_paths[i]);
		}
		free(pref->fallback_font_paths);
	}
	...
	while (m->to != NULL) { free(m->label); free(m->to); ++m; }
	...
	m = pref->project_macros;
	while (m->to != NULL) { free(m->label); free(m->to); ++m; }
	free(pref->project_macros);

	destroy_symmenu(pref->main_symmenu);
	destroy_symmenu(pref->project_macro_menu);
	for (int i = 0; i < 26; ++i) {
		destroy_symmenu(pref->accent_menus[i][0]);
		destroy_symmenu(pref->accent_menus[i][1]);
	}
	
	keymap_t *alts = pref->altsym_entries;
	while (alts->to != NULL) { free(alts->label); free(alts->to); ++alts; }
...
pref_t *read_preferences(const char* filename) {
	...
	prefs->fallback_font_paths = create_string_array(config, "fallback_font_paths", 0, NULL);
	...
	prefs->project_macros = create_keymap_array(config, "project_macros", DEFAULT_PROJECT_MACROS_LEN, DEFAULT_PROJECT_MACROS);
	...
	DEFAULT_LOOKUP(bool, config, "background_keepalive", prefs->background_keepalive, DEFAULT_BACKGROUND_KEEPALIVE);
	...
	prefs->project_macro_menu = create_symmenu_from_keymap(prefs->project_macros, 3);
...
void set_string_array(config_setting_t *root, char const *key, char *const *source) {
	config_setting_t *setting = config_setting_add(root, key, CONFIG_TYPE_ARRAY);
	if (source == NULL) {
		return;
	}
	for (; *source != NULL; ++source) {
		config_setting_t *elem = config_setting_add(setting, NULL, CONFIG_TYPE_STRING);
		config_setting_set_string(elem, *source);
	}
}
...
void set_keymap_array(config_setting_t *root, char const *key, keymap_t const *source) {
	...
	for (; source->to != NULL; ++source) {
		...
		if (source->label != NULL) {
			config_setting_t *label_s = config_setting_add(group, NULL, CONFIG_TYPE_STRING);
			config_setting_set_string(label_s, source->label);
		}
...
void set_symmenu(config_setting_t *root, char const *key, symmenu_t const *source) {
	...
	while (1) {
		...
		if (source->keys[row][col].map->label != NULL) {
			config_setting_t *label_s = config_setting_add(group, NULL, CONFIG_TYPE_STRING);
			config_setting_set_string(label_s, source->keys[row][col].map->label);
		}
...
void save_preferences(pref_t const* prefs, char const* filename) {
	...
	set_string_array(root, "fallback_font_paths", prefs->fallback_font_paths);
	...
	PREF_SET(root, setting, "metamode_hold_key", int, INT, prefs->metamode_hold_key);
	...
	set_keymap_array(root, "project_macros", prefs->project_macros);
	...
	PREF_SET(root, setting, "keyhold_accents", bool, BOOL, prefs->keyhold_accents);
	PREF_SET(root, setting, "background_keepalive", bool, BOOL, prefs->background_keepalive);
	PREF_SET(root, setting, "prefs_version", int, INT, PREFS_VERSION);
