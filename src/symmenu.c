...
SDL_Surface *render_symmenu(SDL_Surface *screen, pref_t *prefs, symmenu_t *menu) {
	...
	if (fg_font == NULL) {
		fg_font = TTF_OpenFont(DEFAULT_FONT_PATH, fg_font_size);
	}
	if (fg_font == NULL) {
		return NULL;
	}
	...
	if (bg_font == NULL) {
		bg_font = TTF_OpenFont(DEFAULT_FONT_PATH, bg_font_size);
	}
	if (bg_font == NULL) {
	...
	if (corner_font == NULL) {
		corner_font = TTF_OpenFont(DEFAULT_FONT_PATH, corner_font_size);
	}
	if (corner_font == NULL) {
		TTF_CloseFont(fg_font);
	...
	for (int row = 0; menu->keys[row] != NULL; ++row) {
		for (int col = 0; menu->keys[row][col].map != NULL; ++col) {
			...
			char const *display_text = sk->map->label ? sk->map->label : sk->map->to;
			int to_len = strlen(display_text);
			free(sk->uc);
			sk->uc = (UChar*)calloc(to_len + 1, sizeof(UChar));
			io_read_utf8_string(display_text, to_len, sk->uc);
		}
	}
