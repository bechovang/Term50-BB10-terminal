...
static TTF_Font* fallback_fonts[8];
static int fallback_font_count = 0;
...
static char application_active = 1;
static char drag_scrolling = 0;
static int last_drag_y = 0;

static SDL_mutex *input_mutex = NULL;

static int event_pipe[2];

static void render_all_symmenus();
...
static void configure_font_face(TTF_Font *target) {
	if (target == NULL) {
		return;
	}
	TTF_SetFontStyle(target, TTF_STYLE_NORMAL);
	TTF_SetFontOutline(target, 0);
	TTF_SetFontKerning(target, 0);
	TTF_SetFontHinting(target, TTF_HINTING_NORMAL);
}

static TTF_Font* get_font_for_glyph(UChar c) {
	if (font != NULL && TTF_GlyphIsProvided(font, c)) {
		return font;
	}
	for (int i = 0; i < fallback_font_count; ++i) {
		if (fallback_fonts[i] != NULL && TTF_GlyphIsProvided(fallback_fonts[i], c)) {
...
static SDL_Surface* render_glyph_surface(UChar c, SDL_Color fg, SDL_Color bg, int style) {
	UChar str[2] = {c, 0};
	TTF_Font *target = get_font_for_glyph(c);
	if (target == NULL) {
		return NULL;
	}
	TTF_SetFontStyle(target, style);
	return TTF_RenderUNICODE_Shaded(target, str, fg, bg);
}

static void open_fallback_fonts(int font_size) {
	fallback_font_count = 0;
	if (prefs->fallback_font_paths == NULL) {
		return;
	}
	for (int i = 0; prefs->fallback_font_paths[i] != NULL && i < 8; ++i) {
		TTF_Font *fallback = TTF_OpenFont(prefs->fallback_font_paths[i], font_size);
		if (fallback != NULL) {
			configure_font_face(fallback);
			fallback_fonts[fallback_font_count++] = fallback;
		}
	}
}
...
int font_init(int font_size){
	...
	/* Set default options */
	configure_font_face(font);
	open_fallback_fonts(font_size);
	...
	str[0] = 'A';
	alt_key_indicator = render_glyph_surface(str[0], metamode_cursor_fg, metamode_cursor_bg, TTF_STYLE_NORMAL);
	...
	str[0] = 'C';
	ctrl_key_indicator = render_glyph_surface(str[0], metamode_cursor_fg, metamode_cursor_bg, TTF_STYLE_NORMAL);
	...
	str[0] = 0x2191;
	shift_key_indicator = render_glyph_surface(str[0], metamode_cursor_fg, metamode_cursor_bg, TTF_STYLE_NORMAL);
	...
	str[0] = 'a';
	altsym_indicator = render_glyph_surface(str[0], metamode_cursor_fg, metamode_cursor_bg, TTF_STYLE_NORMAL);
	...
	str[0] = 'M';
	metamode_cursor = render_glyph_surface(str[0], metamode_cursor_fg, metamode_cursor_bg, TTF_STYLE_NORMAL);
...
void font_uninit(){
	...
	for (int i = 0; i < fallback_font_count; ++i) {
		if (fallback_fonts[i] != NULL) {
			TTF_CloseFont(fallback_fonts[i]);
			fallback_fonts[i] = NULL;
		}
	}
	fallback_font_count = 0;
}
...
static int overlay_button_at(Uint16 x, Uint16 y) {
	int button_size = advance * 3;
	if (button_size < 48) {
		button_size = 48;
	}
	if (x < screen->w - button_size) {
		return 0;
	}
	if (y < button_size) {
		return 1; /* paste */
	}
	if (y > screen->h - button_size) {
		return 2; /* jump live */
	}
	return 0;
}

static void handle_overlay_button(int button_id) {
	switch (button_id) {
	case 1:
...
void handle_mousedown(Uint16 x, Uint16 y){
	int overlay_button = overlay_button_at(x, y);
	if (overlay_button != 0) {
		handle_overlay_button(overlay_button);
		return;
	}
...
void handle_mousemotion(Uint16 x, Uint16 y, Sint16 xrel, Sint16 yrel, Uint8 state) {
	if (!drag_scrolling || current_symmenu != NULL) {
		return;
	}
	int threshold = text_height > 0 ? text_height : 16;
	if ((last_drag_y - y) >= threshold) {
		buf_scroll_view(1);
		last_drag_y = y;
	} else if ((y - last_drag_y) >= threshold) {
		buf_scroll_view(-1);
		last_drag_y = y;
	}
}

void handle_mouseup(Uint16 x, Uint16 y) {
	drag_scrolling = 0;
}
...
static void render_all_symmenus() {
	if (prefs->main_symmenu != NULL) {
		SDL_FreeSurface(prefs->main_symmenu->surface);
		prefs->main_symmenu->surface = render_symmenu(screen, prefs, prefs->main_symmenu);
	}
	if (prefs->project_macro_menu != NULL) {
		SDL_FreeSurface(prefs->project_macro_menu->surface);
		prefs->project_macro_menu->surface = render_symmenu(screen, prefs, prefs->project_macro_menu);
	}
	for (char c = 'a'; c <= 'z'; ++c) {
		size_t idx = (size_t)(c - 'a');
		symmenu_t *m = prefs->accent_menus[idx][0];
		if ((m != NULL) && (m->entries[1].to != NULL)) {
	...
	if (screen_flags & KEY_DOWN) {
		...
		if(metamode && !metamode_just_set){
			...
			if(keys != NULL){
				...
				if(!f && (0 == strncmp(keys, "macro_menu", 10)))       { symmenu_toggle(prefs->project_macro_menu);f=1;}
				if(!f && (0 == strncmp(keys, "scroll_half_page_up", 19)))   { buf_scroll_half_page(-1);f=1;}
				if(!f && (0 == strncmp(keys, "scroll_half_page_down", 21))) { buf_scroll_half_page(1);f=1;}
				if(!f && (0 == strncmp(keys, "scroll_page_up", 14)))   { buf_scroll_page(-1);f=1;}
				if(!f && (0 == strncmp(keys, "scroll_page_down", 16))) { buf_scroll_page(1);f=1;}
				if(!f && (0 == strncmp(keys, "scroll_top", 10)))       { buf_scroll_top();f=1;}
			}
			metamode_toggle();
			return;
		}
...
void render() {
		...
		for(int j = 0; j < cols; ++j){
			...
			if((sc->surface == NULL) && (sc->c != 0)){
				...
				if(buf->inverse_video){
					sc->surface = render_glyph_surface(sc->c, adjust_color(sc->style.bg_color, sc->style), sc->style.fg_color, sc->style.style);
				} else {
					sc->surface = render_glyph_surface(sc->c, adjust_color(sc->style.fg_color, sc->style), sc->style.bg_color, sc->style.style);
				}
				if(sc->surface == NULL){
	...
	if (draw_cursor){
		...
		if(sc->c){
			if(buf->inverse_video){
				inv_cursor = render_glyph_surface(sc->c, adjust_color(sc->style.fg_color, sc->style), sc->style.bg_color, sc->style.style);
			} else {
				inv_cursor = render_glyph_surface(sc->c, adjust_color(sc->style.bg_color, sc->style), sc->style.fg_color, sc->style.style);
			}
			if(inv_cursor == NULL){
				PRINT(stderr, "Rendering failed for char %d\n", (int)sc->c);
			}
	...
	if (altsym_lock) {
		...
		destrect.x = (cols-1) * advance;
		destrect.y = 3 * text_height;
		destrect.w = altsym_indicator->w;
		destrect.h = altsym_indicator->h;
...
int run_render(void* data){
	...
	while(!exit_application){
		...
		if (application_active || !prefs->background_keepalive) {
...
int main(int argc, char **argv) {
	...
	while (!exit_application) {
		...
		case SDL_MOUSEBUTTONUP:
			handle_mouseup(event.button.x, event.button.y);
			break;
		case SDL_MOUSEMOTION:
			handle_mousemotion(event.motion.x, event.motion.y, event.motion.xrel, event.motion.yrel, event.motion.state);
			break;
