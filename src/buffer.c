...
int buf_view_is_live() {
  return buf->top_line == (buf->line - (rows - 1) < 0 ? 0 : buf->line - (rows - 1));
}

void buf_scroll_view(int delta) {
  int max_top = buf->line - (rows - 1);
  if (max_top < 0) {
    max_top = 0;
  }
  ...
  if (buf->top_line < 0) {
    buf->top_line = 0;
  } else if (buf->top_line > max_top) {
    buf->top_line = max_top;
  }
}

void buf_scroll_half_page(int direction) {
  int delta = rows / 2;
  if (delta < 1) {
    delta = 1;
  }
  buf_scroll_view(direction * delta);
}

void buf_scroll_page(int direction) {
  int delta = rows - 1;
  if (delta < 1) {
    delta = 1;
  }
...
void buf_scroll_bottom() {
  int max_top = buf->line - (rows - 1);
  buf->top_line = max_top > 0 ? max_top : 0;
}
